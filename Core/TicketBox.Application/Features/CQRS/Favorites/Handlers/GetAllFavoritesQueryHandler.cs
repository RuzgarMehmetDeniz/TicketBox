...
{
    public class GetAllFavoritesQueryHandler : IRequestHandler<GetAllFavoritesQuery, List<GetFavoriteQueryResult>>
    {
        private readonly IUnitOfWork _unitOfWork;
        private readonly IMapper _mapper;

        public GetAllFavoritesQueryHandler(IUnitOfWork unitOfWork, IMapper mapper)
        {
            _unitOfWork = unitOfWork;
            _mapper = mapper;
        }

        public async Task<List<GetFavoriteQueryResult>> Handle(GetAllFavoritesQuery request, CancellationToken cancellationToken)
        {
            var spec = new FavoriteWithEventSpecification();
